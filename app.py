...
from PyQt6.QtWidgets import QApplication, QMessageBox, QLineEdit, QPushButton, QMainWindow, QDateEdit, QCheckBox, QLabel, QFileDialog, QWidget, QScrollArea, QVBoxLayout
...
class Information(QMainWindow):
    ...
    def showHome(self):
        self.main = MainWindow(self.emailUser)
        self.main.show()
        self.close()
...
class MainWindow(QMainWindow):
    def __init__(self, email):
        ...
        self.email = email
        self.btnAccount = self.findChild(QPushButton, "accountBtn")
        self.btnAccount.clicked.connect(self.show_info)
        
        try:
            with open('data/tour.json', 'r', encoding='utf8') as file:
        ...
        col = 0
        for tour in tours:
            item = TourItem(tour['image'], tour['title'], tour['time'], tour['price'], tour['date'])
    ...
    def show_info(self):
        self.info = Information(self.email)
        self.info.show()
        self.close()

class TourItem(QWidget):
    def __init__(self, image, title, time, price, date):
        super().__init__()
        uic.loadUi("ui/tour_item.ui", self)
        
        self.image = self.findChild(QLabel, "image")
        self.title = self.findChild(QLabel, "title")
        self.time = self.findChild(QLabel, "time")
        self.price = self.findChild(QLabel, "price")
